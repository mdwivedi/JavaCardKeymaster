...
#if 0
     //Intentionally kept here to take help in debugging if needed. As it
     //happens in pretty early boot 
     std::string hex_temp;
        for (auto y : sharingCheck) {
                   std::ostringstream oss2;
                   oss2 << std::hex << std::setw(2) << std::setfill('0') << (unsigned)y;
                  hex_temp += oss2.str();
       }
     LOG(ERROR) << "INS_COMPUTE_SHARED_HMAC_CMD sharingCheck sending to keyStore: " << hex_temp
#endif
