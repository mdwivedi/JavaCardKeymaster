...
/**
 * KMKeymasterApplet implements the javacard applet. It creates repository and
 * other install time objects. It also implements the keymaster state machine
 * and handles javacard applet life cycle events.
 */
public class KMKeymasterApplet extends Applet implements AppletEvent, ExtendedLength {
  ...
  public static final byte[] sharingCheck = { 0x4B, 0x65, 0x79, 0x6D, 0x61, 0x73, 0x74, 0x65, 0x72, 0x20, 0x48, 0x4D,
      0x41, 0x43, 0x20, 0x56, 0x65, 0x72, 0x69, 0x66, 0x69, 0x63, 0x61, 0x74, 0x69, 0x6F, 0x6E };
  // "KeymasterSharedMac"
  public static final byte[] ckdfLable = { 0x4B, 0x65, 0x79, 0x6D, 0x61, 0x73, 0x74, 0x65, 0x72, 0x53, 0x68, 0x61, 0x72,
      0x65, 0x64, 0x4D, 0x61, 0x63 };
  // "Auth Verification"
  public static final byte[] authVerification = { 0x41, 0x75, 0x74, 0x68, 0x20, 0x56, 0x65, 0x72, 0x69, 0x66, 0x69,
      0x63, 0x61, 0x74, 0x69, 0x6F, 0x6E };
  // "confirmation token"
  public static final byte[] confirmationToken = { 0x63, 0x6F, 0x6E, 0x66, 0x69, 0x72, 0x6D, 0x61, 0x74, 0x69, 0x6F,
      0x6E, 0x20, 0x74, 0x6F, 0x6B, 0x65, 0x6E };
  ...
  private static final byte PROVISION_STATUS_BOOT_PARAM = 0x11;
  ...
  private static final byte[] oneHourMsec = { 0, 0, 0, 0, 0, 0x36, (byte) 0xEE, (byte) 0x80 }; // 3600000 msec
  private static final byte[] oneDayMsec = { 0, 0, 0, 0, 0x05, 0x26, 0x5C, 0x00 }; // 86400000 msec
  private static final byte[] oneMonthMsec = { 0, 0, 0, 0, (byte) 0x9A, 0x7E, (byte) 0xC8, 0x00 }; // 2592000000 msec
  private static final byte[] oneYearMsec = { 0, 0, 0, 0x07, 0x57, (byte) 0xB1, 0x2C, 0x00 }; // 31536000000 msec
  // Leap year + 3 yrs
  private static final byte[] fourYrsMsec = { 0, 0, 0, 0x1D, 0x63, (byte) 0xEB, 0x0C, 0x00 }; // 126230400000 msec
  private static final byte[] firstJan2020 = { 0, 0, 0x01, 0x6F, 0x60, 0x1E, 0x5C, 0x00 }; // 1577865600000 msec
  private static final byte[] firstJan2051 = { 0, 0, 0x02, 0x53, 0x27, (byte) 0xC5, (byte) 0x90, 0x00 }; // 2556172800000
                                                                                                         // msec
  ...
  public void process(APDU apdu) {
    ...
    if ((keymasterState == KMKeymasterApplet.INIT_STATE) || (keymasterState == KMKeymasterApplet.INACTIVE_STATE)
    ...
    try {
      ...
      if ((keymasterState == KMKeymasterApplet.ACTIVE_STATE)
          || ((keymasterState == KMKeymasterApplet.IN_PROVISION_STATE) && isProvisioningComplete())) {
        switch (apduIns) {
  ...
  private boolean isProvisioningComplete() {
    if (((provisionStatus & PROVISION_STATUS_ATTESTATION_KEY)
        & (provisionStatus & PROVISION_STATUS_ATTESTATION_CERT_CHAIN)
        & (provisionStatus & PROVISION_STATUS_ATTESTATION_CERT_PARAMS)
        & (provisionStatus & PROVISION_STATUS_SHARED_SECRET)
        & (provisionStatus & PROVISION_STATUS_BOOT_PARAM)) != 0) {
      return true;
    } else {
      return false;
    }
  ...
  private void processDeviceLockedCmd(APDU apdu) {
    ...
    // KMInteger.uint_64(repository.deviceLockedTimestamp, (short)0);
  ...
  /**
   * Receives data, which can be extended data, as requested by the command
   * instance.
   */
  ...
  private void processGetHwInfoCmd(APDU apdu) {
    ...
    final byte[] JavacardKeymasterDevice = { 0x4A, 0x61, 0x76, 0x61, 0x63, 0x61, 0x72, 0x64, 0x4B, 0x65, 0x79, 0x6D,
        0x61, 0x73, 0x74, 0x65, 0x72, 0x44, 0x65, 0x76, 0x69, 0x63, 0x65, };
    ...
    resp.add((short) 1,
        KMByteBlob.instance(JavacardKeymasterDevice, (short) 0, (short) JavacardKeymasterDevice.length));
  ...
  private void processProvisionAttestationCertParams(APDU apdu) {
    ...
    repository.setIssuer(KMByteBlob.cast(tmpVariables[0]).getBuffer(), KMByteBlob.cast(tmpVariables[0]).getStartOff(),
    ...
    repository.setCertExpiryTime(KMByteBlob.cast(tmpVariables[0]).getBuffer(),
        KMByteBlob.cast(tmpVariables[0]).getStartOff(), KMByteBlob.cast(tmpVariables[0]).length());
  ...
  private void processProvisionAttestationCertChainCmd(APDU apdu) {
    ...
    Util.arrayFillNonAtomic(KMByteBlob.cast(context).getBuffer(), KMByteBlob.cast(context).getStartOff(),
        KMByteBlob.cast(context).length(), (byte) 0);
    while (recvLen > 0 && ((short) bytesRead <= bufferLength)) {
      Util.arrayCopyNonAtomic(srcBuffer, srcOffset, buffer, bufferStartOffset, recvLen);
      decoder.incrementalReceiveAndValidateCertificateChain(KMByteBlob.cast(context).getBuffer(),
          KMByteBlob.cast(context).getStartOff(), KMByteBlob.cast(context).length(), buffer, bufferStartOffset,
          recvLen);
  ...
  private void processProvisionSharedSecretCmd(APDU apdu) {
    ...
    // Persist shared Hmac.
    repository.initHmacSharedSecretKey(KMByteBlob.cast(tmpVariables[0]).getBuffer(),
        KMByteBlob.cast(tmpVariables[0]).getStartOff(), KMByteBlob.cast(tmpVariables[0]).length());
  }
  ...
  private void saveAttId(short attTag) {
    ...
    if (tmpVariables[0] != KMType.INVALID_VALUE) {
      ...
      repository.persistAttId(mapToAttId(attTag), KMByteBlob.cast(tmpVariables[0]).getBuffer(),
          KMByteBlob.cast(tmpVariables[0]).getStartOff(), KMByteBlob.cast(tmpVariables[0]).length());
    } else {
      KMException.throwIt(KMError.INVALID_ARGUMENT);
    }
  ...
  private void processDeleteKeyCmd(APDU apdu) {
    ...
    // Receive the incoming request fully from the master.
    receiveIncoming(apdu);
    // Util.arrayFillNonAtomic(scratchPad, (short) 0, (short)
    // apdu.getBuffer().length, (byte) 0);
    // Arguments
    short argsProto = KMArray.instance((short) 1);
    KMArray.cast(argsProto).add((short) 0, KMByteBlob.exp());
    // Decode the argument
    ...
    data[KEY_BLOB] = decoder.decodeArray(tmpVariables[1], KMByteBlob.cast(data[KEY_BLOB]).getBuffer(),
        KMByteBlob.cast(data[KEY_BLOB]).getStartOff(), KMByteBlob.cast(data[KEY_BLOB]).length());
  ...
  private void processComputeSharedHmacCmd(APDU apdu) {
    ...
    while (tmpVariables[2] < tmpVariables[0]) {
      ...
      if (tmpVariables[6] != 0) {
        ...
        Util.arrayCopyNonAtomic(KMByteBlob.cast(tmpVariables[5]).getBuffer(),
            KMByteBlob.cast(tmpVariables[5]).getStartOff(), repository.getHeap(),
      ...
      // Check if the nonce generated here is present in the hmacSharingParameters
      // array.
      // Otherwise throw INVALID_ARGUMENT error.
      if (tmpVariables[7] == 1) {
        if (0 == Util.arrayCompare(repository.getHeap(), (short) (tmpVariables[1] + tmpVariables[3]),
            KMByteBlob.cast(tmpVariables[9]).getBuffer(), KMByteBlob.cast(tmpVariables[9]).getStartOff(),
            tmpVariables[6])) {
    ...
    tmpVariables[6] = seProvider.cmacKdf(KMByteBlob.cast(tmpVariables[8]).getBuffer(),
        KMByteBlob.cast(tmpVariables[8]).getStartOff(), KMByteBlob.cast(tmpVariables[8]).length(), ckdfLable, (short) 0,
        (short) ckdfLable.length, repository.getHeap(), tmpVariables[1], tmpVariables[3], scratchPad, (short) 0);
    ...
    tmpVariables[5] = seProvider.hmacSign(scratchPad, (short) 0, tmpVariables[6], sharingCheck, (short) 0,
        (short) sharingCheck.length, scratchPad, tmpVariables[6]);
  ...
  private void processUpgradeKeyCmd(APDU apdu) {
    ...
    tmpVariables[0] = KMKeyParameters.findTag(KMType.BYTES_TAG, KMType.APPLICATION_ID, data[KEY_PARAMETERS]);
    if (tmpVariables[0] != KMTag.INVALID_VALUE) {
      data[APP_ID] = KMByteTag.cast(tmpVariables[0]).getValue();
    }
    tmpVariables[0] = KMKeyParameters.findTag(KMType.BYTES_TAG, KMType.APPLICATION_DATA, data[KEY_PARAMETERS]);
    if (tmpVariables[0] != KMTag.INVALID_VALUE) {
    ...
    if (tmpVariables[0] != KMType.INVALID_VALUE) {
      // os version in key characteristics must be less the os version stored in
      // javacard or the
      // stored version must be zero. Then only upgrade is allowed else it is invalid
      // argument.
    ...
    if (tmpVariables[1] != KMType.INVALID_VALUE) {
      // The key characteristics should have had os patch level < os patch level
      // stored in javacard
  ...
  private void processImportWrappedKeyCmd(APDU apdu) {
    ...
    if ((tmpVariables[1] == KMType.RSA || tmpVariables[1] == KMType.EC) && (tmpVariables[2] != KMType.RAW)) {
      KMException.throwIt(KMError.UNIMPLEMENTED);
    }
    ...
    parseEncryptedKeyBlob(scratchPad);
    // check whether the wrapping key is RSA with purpose KEY_WRAP, padding RSA_OAEP
    // and Digest
    // SHA2_256.
    ...
    tmpVariables[1] = seProvider.rsaDecipherOAEP256(KMByteBlob.cast(data[SECRET]).getBuffer(),
        KMByteBlob.cast(data[SECRET]).getStartOff(), KMByteBlob.cast(data[SECRET]).length(),
        KMByteBlob.cast(data[PUB_KEY]).getBuffer(), KMByteBlob.cast(data[PUB_KEY]).getStartOff(),
        KMByteBlob.cast(data[PUB_KEY]).length(), KMByteBlob.cast(tmpVariables[0]).getBuffer(),
        KMByteBlob.cast(tmpVariables[0]).getStartOff(), KMByteBlob.cast(tmpVariables[0]).length(), scratchPad,
        (short) 0);
    ...
    if (!seProvider.aesGCMDecrypt(KMByteBlob.cast(data[SECRET]).getBuffer(),
        KMByteBlob.cast(data[SECRET]).getStartOff(), KMByteBlob.cast(data[SECRET]).length(),
        KMByteBlob.cast(data[INPUT_DATA]).getBuffer(), KMByteBlob.cast(data[INPUT_DATA]).getStartOff(),
        KMByteBlob.cast(data[INPUT_DATA]).length(), scratchPad, (short) 0, KMByteBlob.cast(data[NONCE]).getBuffer(),
        KMByteBlob.cast(data[NONCE]).getStartOff(), KMByteBlob.cast(data[NONCE]).length(),
        KMByteBlob.cast(data[AUTH_DATA]).getBuffer(), KMByteBlob.cast(data[AUTH_DATA]).getStartOff(),
        KMByteBlob.cast(data[AUTH_DATA]).length(), KMByteBlob.cast(data[AUTH_TAG]).getBuffer(),
        KMByteBlob.cast(data[AUTH_TAG]).getStartOff(), KMByteBlob.cast(data[AUTH_TAG]).length())) {
  ...
  private void processAttestKeyCmd(APDU apdu) {
    ...
    tmpVariables[0] = KMKeyParameters.findTag(KMType.BOOL_TAG, KMType.DEVICE_UNIQUE_ATTESTATION, data[KEY_PARAMETERS]);
    if (tmpVariables[0] != KMType.INVALID_VALUE) {
      KMException.throwIt(KMError.UNIMPLEMENTED);
    }
    ...
    tmpVariables[0] = KMKeyParameters.findTag(KMType.BYTES_TAG, KMType.ATTESTATION_APPLICATION_ID,
        data[KEY_PARAMETERS]);
    if (tmpVariables[0] == KMType.INVALID_VALUE) {
    ...
    // TODO current assumption is that if active and creation time are missing from
    // characteristics
    // then
    // then it is an error.
    ...
    if (tmpVariables[2] != KMType.INVALID_VALUE) {
      // compare if the expiry time is greater then 2051 then use generalized time
      // format else use
      // utc time format
      ...
      if (KMInteger.compare(tmpVariables[2], tmpVariables[3]) >= 0)
        ...
        tmpVariables[2] = convertToDate(tmpVariables[1], scratchPad, true);
    } else {
      // if no expiry tag is present then use the attestation key certificate's expiry
      // time
      // that was provisioned in the provision command. This will be in Generalized or
      // UTC time
  ...
  private void addAttestationIds(KMAttestationCert cert) {
    final short[] attTags = new short[] { KMType.ATTESTATION_ID_BRAND, KMType.ATTESTATION_ID_DEVICE,
        KMType.ATTESTATION_ID_IMEI, KMType.ATTESTATION_ID_MANUFACTURER, KMType.ATTESTATION_ID_MEID,
        KMType.ATTESTATION_ID_MODEL, KMType.ATTESTATION_ID_PRODUCT, KMType.ATTESTATION_ID_SERIAL };
    byte index = 0;
    short attIdTag;
    ...
    /*
     * if(repository.isAttIdSupported()){ short attTag; short blob; byte index = 0;
     * while (index < repository.ATT_ID_TABLE_SIZE) { if
     * (repository.getAttIdLen(index) != 0) { blob = KMByteBlob.instance(
     * repository.getAttIdBuffer(index), repository.getAttIdOffset(index),
     * repository.getAttIdLen(index)); attTag =
     * KMByteTag.instance(repository.getAttIdTag(index), blob);
     * cert.extensionTag(attTag,true); index++; } } }
     */
  }
  ...
  private short convertToDate(short time, byte[] scratchPad, boolean utcFlag) {
    ...
    Util.arrayCopyNonAtomic(KMInteger.cast(time).getBuffer(), KMInteger.cast(time).getStartOff(), scratchPad,
        (short) (8 - KMInteger.cast(time).length()), KMInteger.cast(time).length());
  ...
  private short makeUniqueId(byte[] scratchPad) {
    ...
    if (tmpVariables[0] == KMType.INVALID_VALUE)
      KMException.throwIt(KMError.ATTESTATION_APPLICATION_ID_MISSING);
    tmpVariables[0] = KMByteTag.cast(tmpVariables[0]).getValue();
    Util.arrayCopyNonAtomic(KMByteBlob.cast(tmpVariables[0]).getBuffer(),
        KMByteBlob.cast(tmpVariables[0]).getStartOff(), scratchPad, tmpVariables[1],
        KMByteBlob.cast(tmpVariables[0]).length());
    tmpVariables[1] += KMByteBlob.cast(tmpVariables[0]).length();
    ...
    // Sign - signature becomes unique id of 32 bits. Use 128 bits master key as an
    // hmac key.
     ...
     */
    tmpVariables[1] = seProvider.hmacSign(KMByteBlob.cast(tmpVariables[2]).getBuffer(),
        KMByteBlob.cast(tmpVariables[2]).getStartOff(), KMByteBlob.cast(tmpVariables[2]).length(), scratchPad,
        (short) 0, tmpVariables[1], KMByteBlob.cast(tmpVariables[0]).getBuffer(),
        KMByteBlob.cast(tmpVariables[0]).getStartOff());
  ...
  private void finishEncryptOperation(KMOperationState op, byte[] scratchPad) {
    ...
    switch (op.getAlgorithm()) {
      ...
      case KMType.RSA:
        ...
        Util.arrayFillNonAtomic(scratchPad, (short) 0, (short) 256, (byte) 0);
        op.getOperation().finish(KMByteBlob.cast(data[INPUT_DATA]).getBuffer(),
            KMByteBlob.cast(data[INPUT_DATA]).getStartOff(), KMByteBlob.cast(data[INPUT_DATA]).length(),
            KMByteBlob.cast(data[OUTPUT_DATA]).getBuffer(), KMByteBlob.cast(data[OUTPUT_DATA]).getStartOff());
        break;
      case KMType.AES:
      case KMType.DES:
        ...
        len = op.getOperation().finish(KMByteBlob.cast(data[INPUT_DATA]).getBuffer(),
            KMByteBlob.cast(data[INPUT_DATA]).getStartOff(), KMByteBlob.cast(data[INPUT_DATA]).length(),
            KMByteBlob.cast(tmpVariables[0]).getBuffer(), KMByteBlob.cast(tmpVariables[0]).getStartOff());

        data[OUTPUT_DATA] = KMByteBlob.instance(KMByteBlob.cast(tmpVariables[0]).getBuffer(),
            KMByteBlob.cast(tmpVariables[0]).getStartOff(), len);
        break;
      default:
    ...
    switch (op.getAlgorithm()) {
      ...
      case KMType.RSA:
        ...
        if (op.getPadding() == KMType.PADDING_NONE && len != 256)
          KMException.throwIt(KMError.INVALID_INPUT_LENGTH);
        len = op.getOperation().finish(KMByteBlob.cast(data[INPUT_DATA]).getBuffer(),
            KMByteBlob.cast(data[INPUT_DATA]).getStartOff(), len, scratchPad, (short) 0);
      ...
      case KMType.DES:
        ...
        if ((op.getBlockMode() == KMType.CBC || op.getBlockMode() == KMType.ECB) && len > 0
          ...
          tmpVariables[0] = op.getOperation().getAESGCMOutputSize(len, (short) (op.getMacLength() / 8));
          tmpVariables[1] = repository.alloc(tmpVariables[0]);
        }
        byte[] heap = repository.getHeap();
        len = op.getOperation().finish(KMByteBlob.cast(data[INPUT_DATA]).getBuffer(),
            KMByteBlob.cast(data[INPUT_DATA]).getStartOff(), len, heap, tmpVariables[1]);
  ...
  // update operation should send 0x00 for finish variable, where as finish
  // operation
  ...
  private void updateAAD(KMOperationState op, byte finish) {
    ...
    // For Finish operation the input data can be zero length and associated data
    // can be
    // INVALID_VALUE
    ...
    // Check if associated data is present and update aad still allowed by the
    // operation.
    if (tmpVariables[1] != KMType.INVALID_VALUE) {
      ...
      op.getOperation().updateAAD(KMByteBlob.cast(tmpVariables[1]).getBuffer(),
          KMByteBlob.cast(tmpVariables[1]).getStartOff(), KMByteBlob.cast(tmpVariables[1]).length());
    }
  }
    ...
    switch (op.getAlgorithm()) {
      case KMType.RSA:
        ...
        if (op.getDigest() == KMType.DIGEST_NONE && op.getPadding() == KMType.PADDING_NONE) {
          ...
          if (op.getPurpose() == KMType.VERIFY && KMByteBlob.cast(data[INPUT_DATA]).length() != 256) {
            KMException.throwIt(KMError.INVALID_INPUT_LENGTH);
          }
        }
        try {
          if (op.getPurpose() == KMType.SIGN) {
            ...
            op.getOperation().sign(KMByteBlob.cast(data[INPUT_DATA]).getBuffer(),
                KMByteBlob.cast(data[INPUT_DATA]).getStartOff(), KMByteBlob.cast(data[INPUT_DATA]).length(),
                KMByteBlob.cast(data[OUTPUT_DATA]).getBuffer(), KMByteBlob.cast(data[OUTPUT_DATA]).getStartOff());
          } else {
            if (!op.getOperation().verify(KMByteBlob.cast(data[INPUT_DATA]).getBuffer(),
                KMByteBlob.cast(data[INPUT_DATA]).getStartOff(), KMByteBlob.cast(data[INPUT_DATA]).length(),
                KMByteBlob.cast(data[SIGNATURE]).getBuffer(), KMByteBlob.cast(data[SIGNATURE]).getStartOff(),
      ...
      case KMType.EC:
        ...
        if (op.getPurpose() == KMType.SIGN) {
          ...
          len = op.getOperation().sign(KMByteBlob.cast(data[INPUT_DATA]).getBuffer(),
              KMByteBlob.cast(data[INPUT_DATA]).getStartOff(), len, scratchPad, (short) 0);
          data[OUTPUT_DATA] = KMByteBlob.instance(scratchPad, (short) 0, len);
        } else {
          if (!op.getOperation().verify(KMByteBlob.cast(data[INPUT_DATA]).getBuffer(),
              KMByteBlob.cast(data[INPUT_DATA]).getStartOff(), len, KMByteBlob.cast(data[SIGNATURE]).getBuffer(),
              KMByteBlob.cast(data[SIGNATURE]).getStartOff(), KMByteBlob.cast(data[SIGNATURE]).length())) {
            KMException.throwIt(KMError.VERIFICATION_FAILED);
          }
        }
        break;
      case KMType.HMAC:
        ...
        // signature manually. The reason for doing this is the TAG_MAC_LENGTH can be 32
        // bytes
        // length or less than that in case if it is less than 32 we are truncating it
        // and sending
        // back to the user. For Verify user will send the truncated and if we pass the
        // truncated
        // signature to javacard verify API it will fail because it expects the full
        // length
        // signature.
        // digest is always present.
        // len of signature will always be 32 bytes.
        op.getOperation().sign(KMByteBlob.cast(data[INPUT_DATA]).getBuffer(),
            KMByteBlob.cast(data[INPUT_DATA]).getStartOff(), KMByteBlob.cast(data[INPUT_DATA]).length(), scratchPad,
            (short) 0);
        ...
        if (op.getPurpose() == KMType.VERIFY) {
          if (0 != Util.arrayCompare(KMByteBlob.cast(data[OUTPUT_DATA]).getBuffer(),
              KMByteBlob.cast(data[OUTPUT_DATA]).getStartOff(), KMByteBlob.cast(data[SIGNATURE]).getBuffer(),
              KMByteBlob.cast(data[SIGNATURE]).getStartOff(), (short) (op.getMacLength() / 8))) {
            KMException.throwIt(KMError.VERIFICATION_FAILED);
          }
        }
        break;
  ...
  private void authorizeDeviceUnlock(short hwToken) {
    // If device is locked and key characteristics requires unlocked device then
    // check whether
    // HW auth token has correct timestamp.
    short ptr = KMKeyParameters.findTag(KMType.BOOL_TAG, KMType.UNLOCKED_DEVICE_REQUIRED, data[HW_PARAMETERS]);
    ...
    if (ptr != KMType.INVALID_VALUE && repository.getDeviceLock()) {
      ...
      /*
       * if (KMInteger.compare(ptr,
       * KMInteger.uint_64(repository.deviceLockedTimestamp, (short) 0)) <= 0) {
       * KMException.throwIt(KMError.DEVICE_LOCKED); }
       */
      if (KMInteger.compare(ptr, ts) <= 0) {
        KMException.throwIt(KMError.DEVICE_LOCKED);
      }
      // Now check if the device unlock requires password only authentication and
      // whether
      ...
      // Util.arrayFillNonAtomic(repository.deviceLockedTimestamp, (short) 0, (short)
      // 8, (byte) 0);
    }
  }
  ...
  private void validateVerificationToken(short verToken, byte[] scratchPad) {
    ...
    /*
     * boolean verified = seProvider.hmacVerify(repository.getComputedHmacKey(),
     * (short) 0, (short) repository.getComputedHmacKey().length, scratchPad,
     * (short) 0, len, KMByteBlob.cast(ptr).getBuffer(),
     * KMByteBlob.cast(ptr).getStartOff(), KMByteBlob.cast(ptr).length());
     * 
     */
    boolean verified = seProvider.hmacVerify(KMByteBlob.cast(key).getBuffer(), KMByteBlob.cast(key).getStartOff(),
        KMByteBlob.cast(key).length(), scratchPad, (short) 0, len, KMByteBlob.cast(ptr).getBuffer(),
        KMByteBlob.cast(ptr).getStartOff(), KMByteBlob.cast(ptr).length());
  ...
  private void processUpdateOperationCmd(APDU apdu) {
    ...
    if (op.getPurpose() == KMType.SIGN || op.getPurpose() == KMType.VERIFY) {
      tmpVariables[0] = KMByteBlob.cast(data[INPUT_DATA]).length();
      // update the data.
      op.getOperation().update(KMByteBlob.cast(data[INPUT_DATA]).getBuffer(),
          KMByteBlob.cast(data[INPUT_DATA]).getStartOff(), KMByteBlob.cast(data[INPUT_DATA]).length());
      ...
      // Update for encrypt/decrypt using RSA will not be supported because to do this
      // op state
      ...
      try {
        tmpVariables[0] = op.getOperation().update(KMByteBlob.cast(data[INPUT_DATA]).getBuffer(),
            KMByteBlob.cast(data[INPUT_DATA]).getStartOff(), KMByteBlob.cast(data[INPUT_DATA]).length(),
            KMByteBlob.cast(data[OUTPUT_DATA]).getBuffer(), KMByteBlob.cast(data[OUTPUT_DATA]).getStartOff());
      } catch (CryptoException e) {
        KMException.throwIt(KMError.INVALID_TAG);
      ...
      if (tmpVariables[0] != KMByteBlob.cast(data[OUTPUT_DATA]).length()) {
        ...
        Util.arrayCopy(KMByteBlob.cast(data[INPUT_DATA]).getBuffer(), KMByteBlob.cast(data[INPUT_DATA]).getStartOff(),
            KMByteBlob.cast(data[OUTPUT_DATA]).getBuffer(), KMByteBlob.cast(data[OUTPUT_DATA]).getStartOff(),
            tmpVariables[0]);
      }
    }
  ...
  private void processBeginOperationCmd(APDU apdu) {
    ...
    parseEncryptedKeyBlob(scratchPad);
    // Authorize the begin operation and reserve op - data[OP_HANDLE] will have the
    // handle.
    ...
    if (data[IV] != KMType.INVALID_VALUE && op.getPurpose() != KMType.DECRYPT && op.getBlockMode() != KMType.ECB) {
      tmpVariables[2] = KMArray.instance((short) 1);
      if (op.getAlgorithm() == KMType.DES && op.getBlockMode() == KMType.CBC) {
        ...
        Util.arrayCopy(KMByteBlob.cast(data[IV]).getBuffer(), KMByteBlob.cast(data[IV]).getStartOff(),
            KMByteBlob.cast(tmpVariables[1]).getBuffer(), KMByteBlob.cast(tmpVariables[1]).getStartOff(), (short) 8);
  ...
  private void beginTrustedConfirmationOperation(KMOperationState op) {
    ...
    if (KMKeyParameters.findTag(KMType.BOOL_TAG, KMType.TRUSTED_CONFIRMATION_REQUIRED,
       ...
       * KMType.VERIFY,KMType.HMAC,KMType.SHA2_256,(byte)0,(byte)0,repository.
       * getComputedHmacKey(), (short) 0, (short)
       * repository.getComputedHmacKey().length,null,(short)0,(short)0,(short)0));
       */
      op.setTrustedConfirmationSigner(seProvider.initSymmetricOperation(KMType.VERIFY, KMType.HMAC, KMType.SHA2_256,
          (byte) 0, (byte) 0, KMByteBlob.cast(key).getBuffer(), KMByteBlob.cast(key).getStartOff(),
          KMByteBlob.cast(key).length(), null, (short) 0, (short) 0, (short) 0));
  ...
  private void authorizeDigest(KMOperationState op) {
    ...
    switch (op.getAlgorithm()) {
      case KMType.RSA:
        if ((paramPadding == KMType.RSA_OAEP || paramPadding == KMType.RSA_PSS) && param == KMType.INVALID_VALUE) {
  ...
  private void authorizePadding(KMOperationState op) {
    ...
    switch (op.getAlgorithm()) {
      case KMType.RSA:
        ...
        if ((op.getPurpose() == KMType.SIGN || op.getPurpose() == KMType.VERIFY) && param != KMType.PADDING_NONE
            && param != KMType.RSA_PSS && param != KMType.RSA_PKCS1_1_5_SIGN)
          KMException.throwIt(KMError.UNSUPPORTED_PADDING_MODE);
        if ((op.getPurpose() == KMType.ENCRYPT || op.getPurpose() == KMType.DECRYPT) && param != KMType.PADDING_NONE
            && param != KMType.RSA_OAEP && param != KMType.RSA_PKCS1_1_5_ENCRYPT)
          KMException.throwIt(KMError.UNSUPPORTED_PADDING_MODE);
        if (param == KMType.PADDING_NONE && op.getDigest() != KMType.DIGEST_NONE) {
          KMException.throwIt(KMError.INCOMPATIBLE_DIGEST);
        }
        if ((param == KMType.RSA_OAEP || param == KMType.RSA_PSS) && op.getDigest() == KMType.DIGEST_NONE) {
          KMException.throwIt(KMError.INCOMPATIBLE_DIGEST);
        }
  ...
  private void authorizeBlockModeAndMacLength(KMOperationState op) {
    ...
    short macLen = KMIntegerTag.getShortValue(KMType.UINT_TAG, KMType.MAC_LENGTH, data[KEY_PARAMETERS]);
    switch (op.getAlgorithm()) {
      case KMType.AES:
        if (param == KMType.INVALID_VALUE)
          KMException.throwIt(KMError.INVALID_ARGUMENT);
        ...
        // else if(macLen != KMType.INVALID_VALUE)
        // KMException.throwIt(KMError.INVALID_ARGUMENT);
        break;
      case KMType.DES:
        if (param == KMType.INVALID_VALUE)
  ...
  private void authorizeAndBeginOperation(KMOperationState op, byte[] scratchPad) {
    ...
    // Authorize Caller Nonce - if caller nonce absent in key char and nonce present
    // in
    // key params then fail if it is not a Decrypt operation
    data[IV] = KMType.INVALID_VALUE;
    tmpVariables[0] = KMKeyParameters.findTag(KMType.BOOL_TAG, KMType.CALLER_NONCE, data[HW_PARAMETERS]);
    ...
    if (tmpVariables[1] != KMType.INVALID_VALUE) {
      ...
      if ((op.getBlockMode() == KMType.CBC || op.getBlockMode() == KMType.CTR) && op.getAlgorithm() == KMType.AES
      ...
      if (op.getPurpose() == KMType.DECRYPT
          && (op.getBlockMode() == KMType.CBC || op.getBlockMode() == KMType.GCM || op.getBlockMode() == KMType.CTR)) {
        ...
        // For encrypt mode if nonce is absent then create random nonce of correct
        // length
        byte ivLen = 16;
        if (op.getBlockMode() == KMType.GCM) {
          ivLen = 12;
        ...
        seProvider.newRandomNumber(KMByteBlob.cast(data[IV]).getBuffer(), KMByteBlob.cast(data[IV]).getStartOff(),
    ...
    switch (op.getAlgorithm()) {
        ...
        try {
          if (op.getPurpose() == KMType.DECRYPT) {
            op.setOperation(seProvider.initAsymmetricOperation((byte) op.getPurpose(), op.getAlgorithm(),
                op.getPadding(), op.getDigest(), KMByteBlob.cast(data[SECRET]).getBuffer(),
                KMByteBlob.cast(data[SECRET]).getStartOff(), KMByteBlob.cast(data[SECRET]).length(),
                KMByteBlob.cast(data[PUB_KEY]).getBuffer(), KMByteBlob.cast(data[PUB_KEY]).getStartOff(),
                KMByteBlob.cast(data[PUB_KEY]).length()));
          } else {
            op.setOperation(seProvider.initAsymmetricOperation((byte) op.getPurpose(), op.getAlgorithm(),
                op.getPadding(), op.getDigest(), null, (short) 0, (short) 0, KMByteBlob.cast(data[PUB_KEY]).getBuffer(),
                KMByteBlob.cast(data[PUB_KEY]).getStartOff(), KMByteBlob.cast(data[PUB_KEY]).length()));
          }
      ...
      case KMType.DES:
        ...
        try {
          ...
          op.setOperation(seProvider.initSymmetricOperation((byte) op.getPurpose(), op.getAlgorithm(), op.getDigest(),
              op.getPadding(), op.getBlockMode(), KMByteBlob.cast(data[SECRET]).getBuffer(),
              KMByteBlob.cast(data[SECRET]).getStartOff(), KMByteBlob.cast(data[SECRET]).length(),
              KMByteBlob.cast(data[IV]).getBuffer(), KMByteBlob.cast(data[IV]).getStartOff(),
              KMByteBlob.cast(data[IV]).length(), op.getMacLength()));
  ...
  private void beginSignVerifyOperation(KMOperationState op) {
    switch (op.getAlgorithm()) {
      ...
      case KMType.EC:
        try {
          if (op.getPurpose() == KMType.SIGN) {
            op.setOperation(seProvider.initAsymmetricOperation((byte) op.getPurpose(), op.getAlgorithm(),
                op.getPadding(), op.getDigest(), KMByteBlob.cast(data[SECRET]).getBuffer(),
                KMByteBlob.cast(data[SECRET]).getStartOff(), KMByteBlob.cast(data[SECRET]).length(), null, (short) 0,
                (short) 0));
          } else {
            op.setOperation(seProvider.initAsymmetricOperation((byte) op.getPurpose(), op.getAlgorithm(),
                op.getPadding(), op.getDigest(), null, (short) 0, (short) 0, KMByteBlob.cast(data[PUB_KEY]).getBuffer(),
      ...
      case KMType.HMAC:
        ...
        try {
          op.setOperation(seProvider.initSymmetricOperation((byte) KMType.SIGN, op.getAlgorithm(), op.getDigest(),
              op.getPadding(), op.getBlockMode(), KMByteBlob.cast(data[SECRET]).getBuffer(),
              KMByteBlob.cast(data[SECRET]).getStartOff(), KMByteBlob.cast(data[SECRET]).length(), null, (short) 0,
              (short) 0, (short) 0));
        } catch (CryptoException exp) {
  ...
  private void authorizeUserSecureIdAuthTimeout(KMOperationState op) {
    ...
    tmpVariables[0] = KMKeyParameters.findTag(KMType.ULONG_ARRAY_TAG, KMType.USER_SECURE_ID, data[HW_PARAMETERS]);
    if (tmpVariables[0] != KMType.INVALID_VALUE) {
      tmpVariables[0] = KMKeyParameters.findTag(KMType.UINT_TAG, KMType.AUTH_TIMEOUT, data[HW_PARAMETERS]);
      if (tmpVariables[0] != KMType.INVALID_VALUE) {
  ...
  private boolean validateHwToken(short hwToken, byte[] scratchPad) {
    ...
    short key = repository.getComputedHmacKey();
    /*
     * return seProvider.hmacVerify( repository.getComputedHmacKey(), (short) 0,
     * (short) repository.getComputedHmacKey().length, scratchPad, (short) 0, len,
     * KMByteBlob.cast(ptr).getBuffer(), KMByteBlob.cast(ptr).getStartOff(),
     * KMByteBlob.cast(ptr).length());
     */
    return seProvider.hmacVerify(KMByteBlob.cast(key).getBuffer(), KMByteBlob.cast(key).getStartOff(),
        KMByteBlob.cast(key).length(), scratchPad, (short) 0, len, KMByteBlob.cast(ptr).getBuffer(),
        KMByteBlob.cast(ptr).getStartOff(), KMByteBlob.cast(ptr).length());
  }
  ...
  private void authorizeKeyUsageForCount() {
    ...
    if (usageLimit == KMType.INVALID_VALUE)
      return;
    // get current counter
  ...
  private void importKey(APDU apdu, byte[] scratchPad) {
    ...
    tmpVariables[0] = KMKeyParameters.findTag(KMType.BOOL_TAG, KMType.BOOTLOADER_ONLY, data[KEY_PARAMETERS]);
    if (tmpVariables[0] != KMType.INVALID_VALUE) {
      KMException.throwIt(KMError.INVALID_KEY_BLOB);
    }
    // Rollback protection not supported
    tmpVariables[0] = KMKeyParameters.findTag(KMType.BOOL_TAG, KMType.ROLLBACK_RESISTANCE, data[KEY_PARAMETERS]);
    if (tmpVariables[0] != KMType.INVALID_VALUE) {
    ...
    // create key blob
    createEncryptedKeyBlob(scratchPad);
    tmpVariables[0] = KMIntegerTag.getShortValue(KMType.UINT_TAG, KMType.MAX_USES_PER_BOOT, data[KEY_PARAMETERS]);
    if (tmpVariables[0] != KMType.INVALID_VALUE) {
  ...
  private void importECKeys(byte[] scratchPad) {
    ...
    tmpVariables[0] = decoder.decode(tmpVariables[0], KMByteBlob.cast(data[IMPORTED_KEY_BLOB]).getBuffer(),
        KMByteBlob.cast(data[IMPORTED_KEY_BLOB]).getStartOff(), KMByteBlob.cast(data[IMPORTED_KEY_BLOB]).length());
    ...
    tmpVariables[2] = KMIntegerTag.getShortValue(KMType.UINT_TAG, KMType.KEYSIZE, data[KEY_PARAMETERS]);
    if (tmpVariables[2] != KMType.INVALID_VALUE) {
      // As per NIST.SP.800-186 page 9, secret for 256 curve should be between
      // 256-383
      if (((256 <= (short) (KMByteBlob.cast(data[SECRET]).length() * 8))
          && (383 >= (short) (KMByteBlob.cast(data[SECRET]).length() * 8))) ^ tmpVariables[2] == 256) {
        KMException.throwIt(KMError.IMPORT_PARAMETER_MISMATCH);
      }
    ...
    if (tmpVariables[3] != KMType.INVALID_VALUE) {
      ...
      // 256-383
      if (((256 <= (short) (KMByteBlob.cast(data[SECRET]).length() * 8))
          && (383 >= (short) (KMByteBlob.cast(data[SECRET]).length() * 8))) ^ tmpVariables[3] == KMType.P_256) {
        KMException.throwIt(KMError.IMPORT_PARAMETER_MISMATCH);
      }
    ...
    seProvider.importAsymmetricKey(KMType.EC, KMByteBlob.cast(data[SECRET]).getBuffer(),
        KMByteBlob.cast(data[SECRET]).getStartOff(), KMByteBlob.cast(data[SECRET]).length(),
        KMByteBlob.cast(data[PUB_KEY]).getBuffer(), KMByteBlob.cast(data[PUB_KEY]).getStartOff(),
        KMByteBlob.cast(data[PUB_KEY]).length());
  ...
  private void importHmacKey(byte[] scratchPad) {
    ...
    // Check whether key can be created
    seProvider.importSymmetricKey(KMType.HMAC, tmpVariables[2], KMByteBlob.cast(data[SECRET]).getBuffer(),
        KMByteBlob.cast(data[SECRET]).getStartOff(), KMByteBlob.cast(data[SECRET]).length());
  ...
  private void importTDESKey(byte[] scratchPad) {
    ...
    // Check whether key can be created
    seProvider.importSymmetricKey(KMType.DES, tmpVariables[2], KMByteBlob.cast(data[SECRET]).getBuffer(),
        KMByteBlob.cast(data[SECRET]).getStartOff(), KMByteBlob.cast(data[SECRET]).length());
  ...
  private void importAESKey(byte[] scratchPad) {
    ...
    // Check whether key can be created
    seProvider.importSymmetricKey(KMType.AES, tmpVariables[2], KMByteBlob.cast(data[SECRET]).getBuffer(),
        KMByteBlob.cast(data[SECRET]).getStartOff(), KMByteBlob.cast(data[SECRET]).length());
  ...
  private void importRSAKey(byte[] scratchPad) {
    ...
    tmpVariables[2] = KMIntegerTag.getValue(scratchPad, (short) 10, // using offset 10 as first 10 bytes reserved for
                                                                    // update params
        KMType.ULONG_TAG, KMType.RSA_PUBLIC_EXPONENT, data[KEY_PARAMETERS]);
    ...
    // Check whether key can be created
    seProvider.importAsymmetricKey(KMType.RSA, KMByteBlob.cast(data[SECRET]).getBuffer(),
        KMByteBlob.cast(data[SECRET]).getStartOff(), KMByteBlob.cast(data[SECRET]).length(),
        KMByteBlob.cast(data[PUB_KEY]).getBuffer(), KMByteBlob.cast(data[PUB_KEY]).getStartOff(),
        KMByteBlob.cast(data[PUB_KEY]).length());
  ...
  private void processSetBootParamsCmd(APDU apdu) {
    ...
    // Decode the arguments
    // System.out.println("Process boot params buffer:
    ...
    repository.setOsVersion(KMInteger.cast(tmpVariables[0]).getBuffer(), KMInteger.cast(tmpVariables[0]).getStartOff(),
        KMInteger.cast(tmpVariables[0]).length());
    repository.setOsPatch(KMInteger.cast(tmpVariables[1]).getBuffer(), KMInteger.cast(tmpVariables[1]).getStartOff(),
    ...
    // .getValue(repository.verifiedBootKey, (short) 0,
    // repository.actualBootKeyLength);
    repository.setVerifiedBootKey(KMByteBlob.cast(tmpVariables[2]).getBuffer(),
        KMByteBlob.cast(tmpVariables[2]).getStartOff(), KMByteBlob.cast(tmpVariables[2]).length());

    // repository.actualBootHashLength = KMByteBlob.cast(tmpVariables[3]).length();
    // KMByteBlob.cast(tmpVariables[3])
    // .getValue(repository.verifiedBootHash, (short) 0,
    // repository.actualBootHashLength);
    repository.setVerifiedBootHash(KMByteBlob.cast(tmpVariables[3]).getBuffer(),
        KMByteBlob.cast(tmpVariables[3]).getStartOff(), KMByteBlob.cast(tmpVariables[3]).length());

    byte enumVal = KMEnum.cast(tmpVariables[4]).getVal();
    repository.setBootState(enumVal);
    /*
     * if (enumVal == KMTag.SELF_SIGNED_BOOT) { repository.selfSignedBootFlag =
     * true; repository.verifiedBootFlag = false; } else if(enumVal ==
     * KMType.VERIFIED_BOOT) { repository.selfSignedBootFlag = false;
     * repository.verifiedBootFlag = true; }else { repository.selfSignedBootFlag =
     * false; repository.verifiedBootFlag = false; }
     * 
     */
  ...
  private static void processGenerateKey(APDU apdu) {
    ...
    tmpVariables[0] = KMKeyParameters.findTag(KMType.BOOL_TAG, KMType.EARLY_BOOT_ENDED, data[KEY_PARAMETERS]);
    if (tmpVariables[0] != KMType.INVALID_VALUE) {
  ...
  private static void validateRSAKey(byte[] scratchPad) {
    // Read key size
    tmpVariables[0] = KMIntegerTag.getShortValue(KMType.UINT_TAG, KMType.KEYSIZE, data[KEY_PARAMETERS]);
    if (tmpVariables[0] == KMTag.INVALID_VALUE) {
      KMException.throwIt(KMError.UNSUPPORTED_KEY_SIZE);
    }
    ...
    tmpVariables[1] = KMIntegerTag.getValue(scratchPad, (short) 0, KMType.ULONG_TAG, KMType.RSA_PUBLIC_EXPONENT,
        data[KEY_PARAMETERS]);
    if ((tmpVariables[1] == KMTag.INVALID_VALUE) || (tmpVariables[1] != 4)) {
      KMException.throwIt(KMError.INVALID_ARGUMENT);
    }
    // Only exponent support is F4 - 65537 which is 0x00010001.
    if (Util.getShort(scratchPad, (short) 0) != 0x01 || Util.getShort(scratchPad, (short) 2) != 0x01) {
      KMException.throwIt(KMError.INVALID_ARGUMENT);
    }
  }
  ...
  private static void generateRSAKey(byte[] scratchPad) {
    ...
    seProvider.createAsymmetricKey(KMType.RSA, KMByteBlob.cast(data[SECRET]).getBuffer(),
        KMByteBlob.cast(data[SECRET]).getStartOff(), KMByteBlob.cast(data[SECRET]).length(),
        KMByteBlob.cast(data[PUB_KEY]).getBuffer(), KMByteBlob.cast(data[PUB_KEY]).getStartOff(),
        KMByteBlob.cast(data[PUB_KEY]).length(), lengths);
  ...
  private static void validateAESKey() {
    ...
    tmpVariables[1] = KMKeyParameters.findTag(KMType.ENUM_ARRAY_TAG, KMType.BLOCK_MODE, data[KEY_PARAMETERS]);
    if (tmpVariables[1] != KMTag.INVALID_VALUE) { // block mode specified
      // Find Minimum Mac length
      tmpVariables[2] = KMKeyParameters.findTag(KMType.UINT_TAG, KMType.MIN_MAC_LENGTH, data[KEY_PARAMETERS]);
      ...
      if (KMEnumArrayTag.cast(tmpVariables[1]).contains(KMType.GCM)) {
        ...
        // Validate the MIN_MAC_LENGTH for AES - should be multiple of 8, less then 128
        // bits
        // and greater the 96 bits
        if (KMInteger.cast(tmpVariables[3]).getSignificantShort() != 0
            || KMInteger.cast(tmpVariables[3]).getShort() > 128 || KMInteger.cast(tmpVariables[3]).getShort() < 96
  ...
  private static void generateAESKey(byte[] scratchPad) {
    ...
    tmpVariables[0] = seProvider.createSymmetricKey(KMType.AES, tmpVariables[0], scratchPad, (short) 0);
  ...
  private static void generateECKeys(byte[] scratchPad) {
    ...
    short[] lengths = tmpVariables;
    seProvider.createAsymmetricKey(KMType.EC, scratchPad, (short) 0, (short) 128, scratchPad, (short) 128, (short) 128,
        lengths);
    // tmpVariables[5] = ((ECPublicKey) ecKey.getPublic()).getW(scratchPad, (short)
    // 0);
    data[PUB_KEY] = KMByteBlob.instance(scratchPad, (short) 128, lengths[1]);
    // tmpVariables[5] = ((ECPrivateKey) ecKey.getPrivate()).getS(scratchPad,
    // (short) 0);
  ...
  private static void validateTDESKey() {
    ...
    tmpVariables[0] = KMIntegerTag.getShortValue(KMType.UINT_TAG, KMType.MIN_MAC_LENGTH, data[KEY_PARAMETERS]);
    if (tmpVariables[0] != KMType.INVALID_VALUE) {
      KMException.throwIt(KMError.INVALID_TAG);
    }
    // Read keysize
    tmpVariables[1] = KMIntegerTag.getShortValue(KMType.UINT_TAG, KMType.KEYSIZE, data[KEY_PARAMETERS]);
    if (tmpVariables[1] == KMType.INVALID_VALUE) {
      KMException.throwIt(KMError.INVALID_ARGUMENT);
    }
  ...
  private static void validateHmacKey() {
    ...
    if (((short) (tmpVariables[0] % 8) != 0) || (tmpVariables[0] < (short) 64) || tmpVariables[0] > (short) 256) {
      KMException.throwIt(KMError.UNSUPPORTED_MIN_MAC_LENGTH);
    }
    // Read keysize
    tmpVariables[1] = KMIntegerTag.getShortValue(KMType.UINT_TAG, KMType.KEYSIZE, data[KEY_PARAMETERS]);
    ...
    if (((short) (tmpVariables[1] % 8) != 0) || (tmpVariables[1] < (short) 64) || tmpVariables[1] > (short) 512) {
      KMException.throwIt(KMError.UNSUPPORTED_KEY_SIZE);
    }
  }
  ...
  private static void generateHmacKey(byte[] scratchPad) {
    ...
    tmpVariables[0] = seProvider.createSymmetricKey(KMType.HMAC, tmpVariables[0], scratchPad, (short) 0);
    data[SECRET] = KMByteBlob.instance(scratchPad, (short) 0, tmpVariables[0]);
    data[KEY_BLOB] = KMArray.instance((short) 4);
  }

  private void checkVersionAndPatchLevel(byte[] scratchPad) {
    tmpVariables[0] = KMIntegerTag.getValue(scratchPad, (short) 0, KMType.UINT_TAG, KMType.OS_VERSION,
        data[HW_PARAMETERS]);
    if (tmpVariables[0] != KMType.INVALID_VALUE) {
      tmpVariables[1] = repository.getOsVersion();
      tmpVariables[1] = Util.arrayCompare(KMInteger.cast(tmpVariables[1]).getBuffer(),
          KMInteger.cast(tmpVariables[1]).getStartOff(), scratchPad, (short) 0, tmpVariables[0]);
      if (tmpVariables[1] == -1) {
    ...
    /*
     * if ((tmpVariables[0] != KMType.INVALID_VALUE) && (Util.arrayCompare(
     * repository.osVersion, (short) 0, scratchPad, (short) 0, tmpVariables[0]) !=
     * 0)) { if (Util.arrayCompare(repository.osVersion, (short) 0, scratchPad,
     * (short) 0, tmpVariables[0]) == -1) { // If the key characteristics has os
     * version > current os version KMException.throwIt(KMError.INVALID_KEY_BLOB); }
     * else { KMException.throwIt(KMError.KEY_REQUIRES_UPGRADE); } }
     */
    tmpVariables[0] = KMIntegerTag.getValue(scratchPad, (short) 0, KMType.UINT_TAG, KMType.OS_PATCH_LEVEL,
        data[HW_PARAMETERS]);
    if (tmpVariables[0] != KMType.INVALID_VALUE) {
    ...
    /*
     * if ((tmpVariables[0] != KMType.INVALID_VALUE) &&
     * (Util.arrayCompare(repository.osPatch, (short) 0, scratchPad, (short) 0,
     * tmpVariables[0]) != 0)) { if (Util.arrayCompare(repository.osPatch, (short)
     * 0, scratchPad, (short) 0, tmpVariables[0]) == -1) { // If the key
     * characteristics has os patch level > current os patch
     * KMException.throwIt(KMError.INVALID_KEY_BLOB); } else {
     * KMException.throwIt(KMError.KEY_REQUIRES_UPGRADE); } }
     */
  }
  ...
  private static void makeKeyCharacteristics(byte[] scratchPad) {
    ...
    tmpVariables[1] = repository.getOsVersion();
    data[HW_PARAMETERS] = KMKeyParameters.makeHwEnforced(data[KEY_PARAMETERS], (byte) data[ORIGIN], tmpVariables[1],
        tmpVariables[0], scratchPad);
  ...
  private static void createEncryptedKeyBlob(byte[] scratchPad) {
    // make key characteristics - returns key characteristics in
    // data[KEY_CHARACTERISTICS]
    makeKeyCharacteristics(scratchPad);
    // make root of trust blob
    // data[ROT] =
    // KMByteBlob.instance(
    // repository.verifiedBootKey, (short) 0, (short)
    // repository.verifiedBootKey.length);
    data[ROT] = repository.getVerifiedBootKey();

    // make hidden key params list
    data[HIDDEN_PARAMETERS] = KMKeyParameters.makeHidden(data[KEY_PARAMETERS], data[ROT], scratchPad);
  ...
  private static void parseEncryptedKeyBlob(byte[] scratchPad) {
    ...
    data[HIDDEN_PARAMETERS] = KMKeyParameters.makeHidden(data[APP_ID], data[APP_DATA], data[ROT], scratchPad);
    // make auth data
    makeAuthData(scratchPad);
  ...
  private static void decryptSecret(byte[] scratchPad) {
    ...
    if (!seProvider.aesGCMDecrypt(repository.getHeap(), data[DERIVED_KEY], tmpVariables[0],
        KMByteBlob.cast(data[SECRET]).getBuffer(), KMByteBlob.cast(data[SECRET]).getStartOff(),
        KMByteBlob.cast(data[SECRET]).length(), scratchPad, (short) 0, KMByteBlob.cast(data[NONCE]).getBuffer(),
        KMByteBlob.cast(data[NONCE]).getStartOff(), KMByteBlob.cast(data[NONCE]).length(), repository.getHeap(),
        data[AUTH_DATA], data[AUTH_DATA_LENGTH], KMByteBlob.cast(data[AUTH_TAG]).getBuffer(),
        KMByteBlob.cast(data[AUTH_TAG]).getStartOff(), KMByteBlob.cast(data[AUTH_TAG]).length())) {
      KMException.throwIt(KMError.INVALID_KEY_BLOB);
    }
    // Copy the decrypted secret
    data[SECRET] = KMByteBlob.instance(scratchPad, (short) 0, KMByteBlob.cast(data[SECRET]).length());
  }
  ...
  private static void encryptSecret(byte[] scratchPad) {
    ...
    Util.arrayCopyNonAtomic(KMByteBlob.cast(data[NONCE]).getBuffer(), KMByteBlob.cast(data[NONCE]).getStartOff(),
        scratchPad, (short) 0, KMByteBlob.cast(data[NONCE]).length());
    seProvider.newRandomNumber(KMByteBlob.cast(data[NONCE]).getBuffer(), KMByteBlob.cast(data[NONCE]).getStartOff(),
        KMByteBlob.cast(data[NONCE]).length());
    // derive master key - stored in derivedKey
    tmpVariables[0] = deriveKey(scratchPad);
    tmpVariables[1] = seProvider.aesGCMEncrypt(repository.getHeap(), data[DERIVED_KEY], tmpVariables[0],
        KMByteBlob.cast(data[SECRET]).getBuffer(), KMByteBlob.cast(data[SECRET]).getStartOff(),
        KMByteBlob.cast(data[SECRET]).length(), scratchPad, (short) 0, KMByteBlob.cast(data[NONCE]).getBuffer(),
        KMByteBlob.cast(data[NONCE]).getStartOff(), KMByteBlob.cast(data[NONCE]).length(), repository.getHeap(),
        data[AUTH_DATA], data[AUTH_DATA_LENGTH], KMByteBlob.cast(data[AUTH_TAG]).getBuffer(),
  ...
  private static void makeAuthData(byte[] scratchPad) {
    tmpVariables[0] = addPtrToAAD(KMKeyParameters.cast(data[HW_PARAMETERS]).getVals(), scratchPad, (short) 0);
    tmpVariables[0] += addPtrToAAD(KMKeyParameters.cast(data[SW_PARAMETERS]).getVals(), scratchPad, tmpVariables[0]);
    tmpVariables[0] += addPtrToAAD(KMKeyParameters.cast(data[HIDDEN_PARAMETERS]).getVals(), scratchPad,
        tmpVariables[0]);
  ...
  private static short deriveKey(byte[] scratchPad) {
    ...
    tmpVariables[3] = seProvider.aesGCMEncrypt(KMByteBlob.cast(tmpVariables[4]).getBuffer(),
        KMByteBlob.cast(tmpVariables[4]).getStartOff(), KMByteBlob.cast(tmpVariables[4]).length(), repository.getHeap(),
        data[AUTH_DATA], data[AUTH_DATA_LENGTH], scratchPad, (short) 0, KMByteBlob.cast(data[NONCE]).getBuffer(),
        KMByteBlob.cast(data[NONCE]).getStartOff(), KMByteBlob.cast(data[NONCE]).length(), repository.getHeap(),
        tmpVariables[1], tmpVariables[2], repository.getHeap(), tmpVariables[5], AES_GCM_AUTH_TAG_LENGTH);
    // Hmac sign.
    tmpVariables[3] = seProvider.hmacSign(repository.getHeap(), tmpVariables[5], AES_GCM_AUTH_TAG_LENGTH, scratchPad,
        (short) 0, tmpVariables[3], repository.getHeap(), tmpVariables[1]);
    if (tmpVariables[3] < 16) {
      KMException.throwIt(KMError.UNKNOWN_ERROR);
    }
    tmpVariables[3] = 16;
    Util.arrayCopyNonAtomic(repository.getHeap(), tmpVariables[1], scratchPad, (short) 0, tmpVariables[3]);
    // store the derived secret in data dictionary
    data[DERIVED_KEY] = repository.alloc(tmpVariables[3]);
    Util.arrayCopyNonAtomic(scratchPad, (short) 0, repository.getHeap(), data[DERIVED_KEY], tmpVariables[3]);
  ...
  private short addIntegers(short num1, short num2) {
    ...
    Util.arrayCopyNonAtomic(KMInteger.cast(num1).getBuffer(), KMInteger.cast(num1).getStartOff(), scratchPad,
        (short) (buf + 8 - KMInteger.cast(num1).length()), KMInteger.cast(num1).length());
    Util.arrayCopyNonAtomic(KMInteger.cast(num2).getBuffer(), KMInteger.cast(num2).getStartOff(), scratchPad,
        (short) (buf + 16 - KMInteger.cast(num2).length()), KMInteger.cast(num2).length());
   ...
   * private short subtractIntegers(short num1, short num2){ short buf =
   * repository.alloc((short)24); byte[] scratchPad = repository.getHeap();
   * Util.arrayFillNonAtomic(scratchPad, buf, (short)24,(byte)0);
   * Util.arrayCopyNonAtomic( KMInteger.cast(num1).getBuffer(),
   * KMInteger.cast(num1).getStartOff(),scratchPad,(short)(buf+8-KMInteger.cast(
   * num1).length()), KMInteger.cast(num1).length()); Util.arrayCopyNonAtomic(
   * KMInteger.cast(num2).getBuffer(),
   * KMInteger.cast(num2).getStartOff(),scratchPad,(short)(buf+16-KMInteger.cast(
   * num2).length()), KMInteger.cast(num2).length()); if(scratchPad[buf] < 0 ||
   * scratchPad[(short)(buf+8)] <0)return KMType.INVALID_VALUE;
   * if(Util.arrayCompare(scratchPad,buf, scratchPad,(short)(buf+8), (short)8) <
   * 1) return KMType.INVALID_VALUE;
   * subtract(scratchPad,buf,(short)(buf+8),(short)(buf+16)); return
   * KMInteger.uint_64(scratchPad,(short)(buf+16)); }
   */
